...
class VideoService {
  ...
  async addVideo({files}) {
    try {
      ...
      writableStream.on('error', error => new ErrorHandler('error while writing video on server', 500))
      writableStream.write(files.video.data)

      return name

    } catch (error) {
      throw new ErrorHandler('error while writing video on server', 500)
    }
  }
  ...
  async getVideoById({params: {id}}, res) {
    try {
      ...
      readableStream.on('error', error => new ErrorHandler('error while writing video on server', 500))
